...
int main(void) {
  ...
  GLuint red_programID =
      LoadShaders("SimpleTransform.glsl", "RedFragmentShader.glsl");

  GLuint green_programID =
      LoadShaders("SimpleTransform.glsl", "GreenFragmentShader.glsl");

  // Get a handle for our "MVP" uniform
  GLuint MatrixID_red = glGetUniformLocation(red_programID, "MVP");
  GLuint MatrixID_green = glGetUniformLocation(red_programID, "MVP");
  ...
  do {
    ...
    glm::mat4 View = glm::lookAt(
        glm::vec3(4*cos(alpha), 3, 4*sin(alpha)),  // Camera starts at (4,3,4), in World Space
    ...
    glUniformMatrix4fv(MatrixID_green, 1, GL_FALSE, &MVP[0][0]);
